...
filepath <<- '/home/freak/Programming/Thesis/results/results/mars_shortterm_aggr/'
...
file.name.aggr.denorm.generic <<- 'mars_shortterm_aggr_combi_aggr_denorm'
...
predict.pow <- function(aggrno) {
  ...
  while(indx.end <= data.len){
    ...
    #With parms: size10-0.943493, size1-0.346469
    #without parms: size10-0.260196, size1-0.183708
    out <<- earth(f,
                  data=data.mat.train)
    ...
    pred <- predict(out, data.mat.test[,1:window.size-1])

...
window.size <- 10
sim.meas <- 'euclidean'
plot.file <- 'mars_shortterm_hclust_aggr_plot.pdf'
...
filepath <<- '/home/freak/Programming/Thesis/results/results/mars_shortterm_hclust/'
...
predict.pow <- function(cut.size, aggrno, indx) {
  ...
  while(indx.end <= data.len){
    ...
    pred <- predict(out, data.mat.test[,1:window.size-1], out$n.trees)
